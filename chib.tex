...
\chibThreeP yields are the expected smooth decreasing functions of $\pt^{\Y1S}$.
...
The  \chiboneTwoP peak width depends on $\pt^{\Y2S}$ interval and is fixed to
the value obtained from simulation~\ref{sec:x}. The \chiboneThreeP peak widths
is fixed to \chiboneTwoP peak width scaled by 1.65.

The ratio of \chiboneThreeP and \chiboneTwoP peak
widths is also obtained from simulation and fixed to 1.65.
...
was measured in $18 < p_T^{\Y2S} < 40 \gevc$ for merged \sqs=7 and
\sqs=8 \tev data.

Figure~\ref{fig:chib:ups2s:yields} shows the number of signal events as function
of $\pt^{\Y2S}$. Table~\ref{tab:chib:ups2s:fits} in Appendix summarizes the
obtained results.

\input{pics/chib/ups2s_yields}

Figure ~\ref{fig:chib:ups2s:yields} shows that \chibOneP and
\chibThreeP yields are smooth decreasing functions of $\pt^{\Y1S}$ and some
 fluctuations are observed for \chibTwoP yields.

% Figure~\ref{fig:chib-2s:mass} shows that \chiboneTwoP mass,
% measured in different $p_T^{\TwoS}$ intervals, variates in the  range
% from 10.243 to 10.252 \gevcc. The PDG mass of \chiboneTwoP is equal to 10.255 \gevcc
% and it is a little higher than measured mass.

% \input{chib/2s/pics/mass}

% Table~\ref{tab:chib-2s:bins-short} lists the yields and other fit parameters
% in different intervals of $p_T^{\TwoS}$.

% \input{chib/2s/tbls/bins}
...
% \input{chib/1s/tbls/bins}

% The  good agreement between yields obtained for different energies is observed
% if it will be scaled by luminosity value.



% \input{chib/fit/index}
% \input{chib/1s/index}
% \input{chib/2s/index}
% \input{chib/3s/index}