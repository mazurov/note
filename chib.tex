...
difference in 10.5\mevcc~\cite{Motyka:1997di}. The $\lambda$ parameter depends on
the $p_T(\Upsilon)$ range and is fixed to the value that is based on the
theoretical prediction discussed in Section~\ref{sec:ratio}. 
...
was consequently fixed to the measured value of 10.511\gevcc in these studies. 
...
the value obtained from simulation (Section~\ref{sec:mc}). 
...
the following, this mass was fixed to 9.892 \gevcc which is the value 
measured on the combined 2011 and 2012 datasets in the range $14<p_T^{\Y1S}<40\gevc$.
...
$p_T$ bin is due to the fit in the region, that don't distinctly separate the background and signal (see~\Cref{fig:chib:ups1s:fits2011,fig:chib:ups1s:fits2012}).
...
\input{tables/chib/converted_ratio}
...
\input{pics/chib/ups1s_converted_fits}
\input{tables/chib/ups1s_conv_fits}

\Cref{fig:chib:ups1s:conv_mean} shows that in $p_T^{\Y1S} > 14 \gevc$ range the
mass of $\chiboneOneP$ is consistent with the PDG value. 

\input{pics/chib/ups1s_conv_mean}

The difference between \chibOneP yields obtained from base
fits~(\Cref{tab:chib:ups1s:fits}) and yields from
{\Cref{tab:chib:ups1s:conv_fits} is around 3\%.  Therefore, the conclusion can
be made that the fit model for $\chibOneP \to \Y1S \gamma$ decay where the
fractions of \chiboneOneP and \chibtwoTwoP are constrained from the study with
converted photons does not significantly change the results for the base fits,
where these fractions are considered to be equal. %%

\subsection{\texorpdfstring{\chib}{xb} yields in     \texorpdfstring{$\chib \to
\Y2S \gamma$}{xb --> Y(2S) gamma} decays} \label{sec:chib:ups2s:fit}
...
In the following analysis this mass was fixed to 10.251\gevcc, which was measured
...
A forth order polynomial for the background (described in Equation 5.3) is used.
...
used for \chiboneThreeP mass estimation. 
The \chiboneThreeP mass is measured to be $10{,}511.3 \pm
1.7\stat \mevcc$, where the combined 2011 and 2012 datasets are used, 
the central value has been obtained by setting $\lambda = 0.67$ in the fit.
% and the 
...
% This result is also compatible within~$\sim{}1.5\sigma$ with 
